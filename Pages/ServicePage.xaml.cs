...
public partial class ServicePage : UserControl
{
    ...
    private void BuildUi()
    {
        ...
        // Tools with console
        root.Children.Add(Section("Инструменты (вывод в консоль)"));
        var toolsCard = Card();
        var toolsBtns = new WrapPanel();
        toolsBtns.Children.Add(ActionBtn("🔍 Диагностика", async () => await RunBridge("RunDiagnostics"), true));
        toolsBtns.Children.Add(ActionBtn("🧪 Тестирование стратегий", async () => await RunBridge("RunTests"), true));
        toolsCard.Child = toolsBtns;
        root.Children.Add(toolsCard);
        root.Children.Add(new TextBlock
        {
            Text = "В тестах: пункт 1 (HTTP/HTTPS) → 2 (Scan). Для Ростелеком/МГТС введите номера: 1,5,8,11,19",
            Foreground = (Brush)Application.Current.FindResource("TextMutedBrush"),
            TextWrapping = TextWrapping.Wrap,
            FontSize = 12,
        ...
        root.Children.Add(new TextBlock
        {
            Text = "Операции со службой и диагностика требуют прав администратора.",
            Foreground = (Brush)Application.Current.FindResource("TextMutedBrush"),
            FontSize = 12,
