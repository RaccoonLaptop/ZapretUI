@echo off
chcp 65001 > nul
:: Niko_ALT11 — авторский конфиг Niko (CDPi UI / Zapret UI)
...
--filter-udp=%GameFilterUDP% --ipset="%LISTS%ipset-all.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude-user.txt" --dpi-desync=fake --dpi-desync-repeats=10 --dpi-desync-any-protocol=1 --dpi-desync-fake-unknown-udp="%BIN%stun.bin" --dpi-desync-cutoff=n4
